...
export const deleteAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  export const fetchAllUsers = catchAsyncErrors(async (req, res, next) => {
    const users = await User.aggregate([ //aggregate-> to sort users like bidder, auctionner.
      {
        $group: {
          _id: {
          ...
          count: { $sum: 1 }, //count of how many users are registered
        },
      },
      {
        $project: { //reshapes the above data
          month: "$_id.month",
          year: "$_id.year",
          role: "$_id.role",
          count: 1,
